...
const newPkg = {
  ...
  scripts: {
    ...
    'test:e2e': 'cross-env NODE_ENV=test NODE_OPTIONS=\"--enable-source-maps\" node --test --test-concurrency=1 --import tsx \"src/tests/e2e/**/*.e2e.ts\"',
    'test:e2e:file': 'cross-env NODE_ENV=test NODE_OPTIONS=\"--enable-source-maps\" node --test --import tsx',
    'test:unit': 'cross-env NODE_ENV=test NODE_OPTIONS=\"--enable-source-maps\" node --test --import tsx \"src/tests/unit/**/*.{test,spec}.ts\"',
